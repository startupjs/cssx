...
The preset accepts these options:

```js
['cssxjs/babel', {
  // Platform: 'ios', 'android', 'web' (auto-detected in most cases)
  platform: undefined,

  // Force React type: 'react-native' or 'web' (auto-detected)
  reactType: undefined,

  // Enable caching library: 'teamplay' (see Caching guide)
  cache: undefined,

  // Transform pug`` templates (default: true)
  transformPug: true,

  // Transform styl`` templates (default: true)
  transformCss: true
}]
```
...
CSSX works on web through [react-native-web](https://necolas.github.io/react-native-web/). If you're using Expo, web support is included automatically.

For custom webpack setups, ensure `react-native-web` is properly configured and aliased.
